...
void priorityEnqueue_PCB(struct Queue_PCB *const queue, struct Node_PCB *const PCB, int priority)
{
    ...
    else if(queue->start->priority > priority)
    {
        PCB->next = queue->start;
        queue->start = PCB;
    }
    else
    {
...
void timeEnqueue_PCB(struct Queue_PCB *const queue, struct Node_PCB *const PCB, int time)
{
    if (queue->end == NULL)
    {
        queue->start = PCB;
    ...
    else if(queue->start->remainingTime > time)
    {
        PCB->next = queue->start;
        queue->start = PCB;
    }
